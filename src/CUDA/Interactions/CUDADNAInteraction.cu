	...
	if(_use_edge) {
		if(_n_forces == 1) { // we can directly use d_forces and d_torques so that no sum is required
			dna_forces_edge_nonbonded
				...
				_grooving, _use_debye_huckel, _use_oxDNA2_coaxial_stacking, _update_st, _d_st, d_box, _d_particle_filter, _filter_requires_dna);
		}
		...
		dna_forces_edge_bonded
			...
			_mbf_finf, _update_st, _d_st, _d_particle_filter, _filter_requires_dna);
	}
	else {
		dna_forces
			<<<_launch_cfg.blocks, _launch_cfg.threads_per_block>>>
			...
			_mbf_xmax, _mbf_finf, _update_st, _d_st, d_box, _d_particle_filter, _filter_requires_dna);
		CUT_CHECK_ERROR("forces_second_step simple_lists error");
	}
}

void CUDADNAInteraction::_hb_op_precalc(c_number4 *poss, GPU_quat *orientations, int *op_pairs1, int *op_pairs2, float *hb_energies, int n_threads, bool *region_is_nearhb, CUDA_kernel_cfg ffs_hb_precalc_kernel_cfg, CUDABox*d_box) {
	hb_op_precalc<<<ffs_hb_precalc_kernel_cfg.blocks, ffs_hb_precalc_kernel_cfg.threads_per_block>>>(poss, orientations, op_pairs1, op_pairs2, hb_energies, n_threads, region_is_nearhb, d_box);
	CUT_CHECK_ERROR("hb_op_precalc error");
}

void CUDADNAInteraction::_near_hb_op_precalc(c_number4 *poss, GPU_quat *orientations, int *op_pairs1, int *op_pairs2, bool *nearly_bonded_array, int n_threads, bool *region_is_nearhb, CUDA_kernel_cfg ffs_hb_precalc_kernel_cfg, CUDABox*d_box) {
	near_hb_op_precalc<<<ffs_hb_precalc_kernel_cfg.blocks, ffs_hb_precalc_kernel_cfg.threads_per_block>>>(poss, orientations, op_pairs1, op_pairs2, nearly_bonded_array, n_threads, region_is_nearhb, d_box);
	CUT_CHECK_ERROR("nearhb_op_precalc error");
}

void CUDADNAInteraction::_dist_op_precalc(c_number4 *poss, GPU_quat *orientations, int *op_pairs1, int *op_pairs2, c_number *op_dists, int n_threads, CUDA_kernel_cfg ffs_dist_precalc_kernel_cfg, CUDABox*d_box) {
	dist_op_precalc<<<ffs_dist_precalc_kernel_cfg.blocks, ffs_dist_precalc_kernel_cfg.threads_per_block>>>(poss, orientations, op_pairs1, op_pairs2, op_dists, n_threads, d_box);
